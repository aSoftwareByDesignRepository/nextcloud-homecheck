#!/usr/bin/env node
/**
 * Behavioural tests for HomeCheck app-feedback.js (mailto + toast hook).
 *
...
function boot(extra) {
	...
	const config = {
		appId: 'homecheck',
		appDisplayName: 'HomeCheck',
		appVersion: '1.0.0',
		feedbackEmail: 'dev@software-by-design.de',
		githubIssuesUrl: 'https://github.com/aSoftwareByDesignRepository/nextcloud-homecheck/issues',
		cssPrefix: 'hmk',
		...(extra || {}),
	};
	const cfgEl = {
	...
	nodes.set('hmk-app-feedback-config', cfgEl);
	...
	nodes.set('hmk-feedback-problem', problem);
	nodes.set('hmk-feedback-idea', idea);

	const trigger = {
		className: 'hmk-nav-footer__trigger',
		getAttribute(k) {
			return this[k] || null;
		},
		setAttribute(k, v) {
	...
	trigger['aria-controls'] = 'hmk-feedback-menu';
	const menu = {
		id: 'hmk-feedback-menu',
		hidden: true,
		contains() {
			return false;
		},
	...
	nodes.set('hmk-feedback-menu', menu);
	doc.querySelector = (sel) => {
		if (sel === '.hmk-nav-footer__trigger') {
			return trigger;
		}
		return null;
	};
	doc.getElementById = (id) => {
		if (id === 'hmk-feedback-menu') {
			return menu;
		}
		return nodes.get(id) || null;
	};
	...
	const sandbox = {
		...
		location: {
			...
			pathname: '/apps/homecheck/today',
			search: '?token=secret&keep=1',
			href: 'https://cloud.example/apps/homecheck/today?token=secret&keep=1',
		},
...
function main() {
	...
	const clean = sandbox.SbdAppFeedback.sanitizePageUrl(
		'https://cloud.example/apps/homecheck/x?token=abc&keep=1&password=no'
	);
	...
	const upgradeAck = sandbox.SbdAppFeedback.sanitizePageUrl(
		'/apps/homecheck/?IKnowThatThisIsABigInstanceAndTheUpdateRequestCouldRunIntoATimeoutAndHowToRestoreABackup=IAmSuperSureToDoThis'
	);
	assert(upgradeAck === '/apps/homecheck/', 'strips Nextcloud web-upgrade ack query key');
	...
	process.stdout.write('app-feedback.test.js OK (homecheck)\n');
}
