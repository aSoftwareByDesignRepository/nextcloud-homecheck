 ...
 * @var \OCP\IL10N $l
 * @var \OCA\HomeCheck\Support\AppFeedbackLinks $appFeedbackLinks optional; constructed when omitted
...
$l = $l ?? (\OCP\Util::getL10N('homecheck'));
...
if ($version === '' && class_exists(\OCP\Server::class)) {
	try {
		...
		$resolved = trim((string)$appManager->getAppVersion('homecheck'));
		if ($resolved !== '') {
			$version = $resolved;
		}
	} catch (\Throwable) {
...
if (!isset($appFeedbackLinks) || !$appFeedbackLinks instanceof AppFeedbackLinks) {
	$appFeedbackLinks = new AppFeedbackLinks('homecheck', 'HomeCheck', $version);
}
$pageUrl = '';
